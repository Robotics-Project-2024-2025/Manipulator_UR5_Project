...
\begin{itemize}
    ...
    \item A bug in the Gazebo management was encountered: if a goal is sent via message, the client successfully sends it, the server receives and accepts it, but it gets stuck indefinitely. This was solved with a bash line called from C++, but the line is quite large in size, and during various tests, we encountered errors not so much due to the algorithm of manipulation, but more because the computed trajectories are partially resolved, as if they are truncated. The malfunctioning of the complete algorithm could also be attributed to this factor (solved, but may have lead to this bug).
    \item Managing the robot’s movement by avoiding singularities and obstacles such as the table or columns behind was done by defining the operational space. These limits lead to drawbacks in the inverse kinematics, because, due to these constraints, sometimes the system could not perform  these trajectories. (done, but leads to malfunctioning)
    \item Managing the various communications between nodes and packages, including the creation of multiple blocks at the same time (done).
\end{itemize}

% Conclusion
\section{Conclusion}\label{sec:conclusion}
In conclusion we were not able to provide a functional implementation of the project. The vision part works fine and has a good expandability since a different number of blocks can be generated and this can be modified in future implementations. For the manipulation part the issue with the movement of the arm that gets stuck is not solved. This may be because of a problem of the server, but since we don't know for sure the idea was to try to implement it with differential kinematics but the time did not allow us to try it. This can be a challenge for the future...
