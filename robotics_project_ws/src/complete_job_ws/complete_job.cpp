...
public:
    ...
    TrajectoryActionClient() : Node("trajectory_action_client")
    {
        ...
        /*
        open_depth_client = this->create_client<serviceDepth>("service_depth");

        if (!open_depth_client_->wait_for_service(10s)){
            RCLCPP_ERROR(this->get_logger(), "Depth calculator not available after waiting");
            rclcpp::shutdown();
            return;
        }

        auto request = std::make_shared<serviceDepth>();
        open_depth_client_->async_send_request(request);
        */
